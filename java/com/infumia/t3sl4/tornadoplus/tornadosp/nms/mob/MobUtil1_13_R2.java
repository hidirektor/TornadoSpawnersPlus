...
public class MobUtil1_13_R2 implements IMobUtil {
   public Entity createEntity(EntityType entityType, World world) {
      WorldServer worldServer = ((CraftWorld)world).getHandle();
      EntityLiving entity = (EntityLiving)EntityTypes.a(worldServer, new MinecraftKey(entityType.name().toLowerCase(Locale.ENGLISH)));
   ...
   public Object convertNBT(IEntity iEntity) {
      ...
      armorTag = new NBTTagCompound();
      armorTag.setString("Name", GenericAttributes.h.getName());
   ...
   public Creature addTarget(List<EntityType> targets, Creature creature) {
      ...
      Class entityCreatureClass;
      for(Iterator var7 = targets.iterator(); var7.hasNext(); entityCreature.targetSelector.a(2, new PathfinderGoalNearestAttackableTarget(entityCreature, entityCreatureClass, true))) {
         EntityType entityType = (EntityType)var7.next();
         entityCreatureClass = null;

         try {
            entityCreatureClass = EntityTypes.a(entityType.name().toLowerCase(Locale.ENGLISH)).c();
