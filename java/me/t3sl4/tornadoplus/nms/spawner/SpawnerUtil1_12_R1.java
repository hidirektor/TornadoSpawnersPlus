...
public class SpawnerUtil1_12_R1 implements ISpawnerUtil {
   public void placeSpawner(ISpawner iSpawner, Location location) {
      ...
      if (PluginUtil.getInstance().getCustomVersion().equalsIgnoreCase("1.12R1")) {
         ReflectionUtil.getInstance().exec(spawner, "a", nbtTileEntity);
      } else {
   ...
   private NBTTagCompound spawnData(Location location) {
      ...
      return spawner == null ? null : (NBTTagCompound)spawner.d().get("SpawnData");
   }
