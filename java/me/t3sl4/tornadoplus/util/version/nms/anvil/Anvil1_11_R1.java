...
public class Anvil1_11_R1 implements IAnvil {
   ...
   public void setActiveContainerId(Object containerAnvil, int containerId) {
      ((ContainerAnvil)containerAnvil).windowId = containerId;
   }

   public Inventory toBukkitInventory(Object containerAnvil) {
      return ((ContainerAnvil)containerAnvil).getBukkitView().getTopInventory();
   }

   public void addActiveContainerSlotListener(Object containerAnvil, Player player) {
      ((ContainerAnvil)containerAnvil).addSlotListener(this.toNMS(player));
   }

   public ContainerAnvil newContainerAnvil(Player player) {
      return new Anvil1_11_R1.AnvilContainer(this.toNMS(player), (Anvil1_11_R1.AnvilContainer)null);
   }
   ...
   private class AnvilContainer extends ContainerAnvil {
      private AnvilContainer(EntityPlayer entityPlayer) {
         super(entityPlayer.inventory, entityPlayer.getWorld(), BlockPosition.ZERO, entityPlayer);
         this.checkReachable = false;
      }

      // $FF: synthetic method
      AnvilContainer(EntityPlayer var2, Anvil1_11_R1.AnvilContainer var3) {
         this(var2);
      }
   }
}
